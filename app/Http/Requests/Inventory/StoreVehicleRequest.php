...
class StoreVehicleRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            'branch_id' => [$isDraft ? 'nullable' : 'required', 'integer', 'exists:branches,id'],
            'make_id' => [$isDraft ? 'nullable' : 'required', 'integer', 'exists:makes,id'],
            'model_id' => [$isDraft ? 'nullable' : 'required', 'integer', 'exists:models,id'],
            'stock_number' => [$isDraft ? 'nullable' : 'required', 'string', 'max:255'],
            'vin' => [$isDraft ? 'nullable' : 'required', 'string', 'max:255'],
            'year' => [$isDraft ? 'nullable' : 'required', 'integer', 'between:1900,2030'],
            'title' => [$isDraft ? 'nullable' : 'required', 'string', 'max:255'],
            'slug' => ['nullable', 'string', 'max:255', 'unique:vehicles,slug'],
            'sale_price' => [$isDraft ? 'nullable' : 'required', 'numeric', 'min:0'],
