...
export default function Show({ invoice }: { invoice: Invoice }) {
  const outstandingBalance = (invoice.total ?? 0) - (invoice.paid_amount ?? 0);
  ...
  return (
    <PaymentShell title={`Invoice ${invoice.invoice_number ?? `INV-${invoice.id}`}`} description={`Updated ${formatDateTime(invoice.updated_at)}`} actions={<><PaymentBackButton /><Button variant="outline" onClick={() => window.print()}><Printer className="mr-2 size-4" />Print</Button></>}>
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card><CardHeader><CardTitle>Payment status</CardTitle></CardHeader><CardContent className="space-y-2"><PaymentStatusBadge status={invoice.status} /><p className="text-sm text-muted-foreground">Due date</p><p>{formatDateTime(invoice.due_at)}</p><p className="text-sm text-muted-foreground">Created</p><p>{formatDateTime(invoice.created_at)}</p></CardContent></Card>
        ...
        <Card className="lg:col-span-2"><CardHeader><CardTitle>Financial breakdown</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><div><p className="text-sm text-muted-foreground">Subtotal</p><p className="font-semibold">{formatCurrency(invoice.subtotal)}</p></div><div><p className="text-sm text-muted-foreground">Taxes</p><p className="font-semibold">{formatCurrency(invoice.tax_total)}</p></div><div><p className="text-sm text-muted-foreground">Discounts</p><p className="font-semibold">{formatCurrency(0)}</p></div><div><p className="text-sm text-muted-foreground">Total</p><p className="font-semibold">{formatCurrency(invoice.total)}</p></div></CardContent></Card>
