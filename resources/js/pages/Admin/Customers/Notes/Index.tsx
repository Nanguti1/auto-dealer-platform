  ...
  if (isLoading) {
    return (
      <CustomerShell title="Customer Notes" description="Manage customer notes and timeline context." actions={<Button asChild><Link href={`${base}/create`}>Create Note</Link></Button>}>
        <LoadingState message="Loading notes..." variant="full-page" />
      </CustomerShell>
    );
  }
    ...
    <CustomerShell title="Customer Notes" description="Manage customer notes and timeline context." actions={<Button asChild><Link href={`${base}/create`}>Create Note</Link></Button>}>
      {notes.length === 0 ? (
        <EmptyGeneric
          ...
          description="No customer notes have been created yet."
          action={{ label: 'Create note', onClick: () => router.visit(`${base}/create`) }}
        />
      ) : (
